...
*/
export const SearchResultsWindow = GObject.registerClass(
  {
  ...
  class SearchResultsWindow extends Adw.PreferencesWindow {
    ...
    async _findLocation() {
      ...
      if (this._provider === GeolocationProvider.OPENSTREETMAPS) {
        ...
        try {
          json = await this._loadJsonAsync(_osmUrl, params).then(
            async (json) => {
              if (!json) {
      ...
      // MapQuest
        ...
        try {
          json = await this._loadJsonAsync(_mqUrl, params).then(
            async (json) => {
              if (!json) {
                this._resultsError(true);
                throw new Error("Server returned an invalid response");
